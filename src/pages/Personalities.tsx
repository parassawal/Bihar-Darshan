        ...
        {filteredData.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
            {filteredData.map((person) => (
              <div key={person.id} className="group relative bg-white rounded-[2rem] overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500 flex flex-col h-[550px]">
                {/* Image Wrapper */}
                <div className="relative h-2/3 overflow-hidden bg-slate-100">
                  ...
                  />
                  {/* Category Badge */}
                  <div className="absolute top-5 left-5 z-20 flex items-center gap-2 bg-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-tighter text-slate-900 shadow-xl">
                     <span className="w-2 h-2 rounded-full bg-orange-500 animate-pulse"></span> {person.category}
                  </div>
                </div>
                ...
                <div className="p-8 flex flex-col justify-between flex-grow bg-white z-10">
                  <div>
                    <span className="text-orange-600 text-[11px] font-black uppercase mb-1 flex items-center gap-1.5">
                      <MapPin size={12} fill="currentColor" /> {person.district}, Bihar
                    </span>
                    <h3 className="text-2xl font-extrabold text-slate-900 mb-3 group-hover:text-orange-600 transition-colors">{person.name}</h3>
